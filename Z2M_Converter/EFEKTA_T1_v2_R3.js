...
const tzLocal = {
	...
	termostat_config: {
        key: ['high_temp', 'low_temp', 'enable_temp', 'invert_logic_temp', 'resolution'],
        convertSet: async (entity, key, rawValue, meta) => {
            const lookup = {'OFF': 0x00, 'ON': 0x01};
            const value = lookup.hasOwnProperty(rawValue) ? lookup[rawValue] : parseInt(rawValue, 10);
            const payloads = {
...
const definition = {
        ...
        configure: async (device, coordinatorEndpoint, logger) => {
            ...
            await reporting.bind(endpointOne, coordinatorEndpoint, ['genPowerCfg', 'genTime', 'msTemperatureMeasurement']);
			const overrides1 = {min: 900, max: 43200, change: 1};
			const overrides2 = {min: 30, max: 1200, change: 10};
